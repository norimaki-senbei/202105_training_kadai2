...
const BadReport = require('./models').BadReport;
  ...
  case 'GET':
    ...
    BadReport.findAll().then((reports) => {
      ...
      const forEachPromise = new Promise((resolve, reject) => {
        ...
        //resolveが非同期になっちゃってる。forEachを待つ方法は？？？
        setTimeout(function () {
          resolve(reports);  
      }, 1000);//Promiseうまく使えてないので、とりあえず1秒待つ
      });
      
      forEachPromise.then((completeReports) => {
        //ぱぐにおくるーー
        console.log("パグに行きまーーす");
