...
const { report } = require('../../kadai2-schedule-arranger/routes/comments');
  ...
  switch (req.method) {
    ...
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        const matchResult = decoded.match(/submitTitle=(.*)&content=(.*)&oneTimeToken=(.*)/);
        if (!matchResult) {
          util.handleBadRequest(req, res);
        } else {
          const submitTitle = matchResult[1];
...
function handleBadReports(req, res) {
  switch (req.method) {
  case 'GET':
    res.writeHead(200, {
      'Content-Type': 'text/html; charset=utf-8'
    });
    BadReport.findAll().then((reports) => {
      //Postテーブルからcontentを引っ張る
      let displayContent = [];
      reports.forEach((report) => {
        Post.findByPk(report.postId).then(getPost => {
          console.log(getPost.content)
          displayContent.push(getPost.content);
      });
      });

      //for (var i = 0; i < reports.length; i++){
      //  Post.findByPk(reports[i].postId).then(getPost => {
      //    console.log(getPost.content)
      //    reports[i].content = getPost.content;
      //});
      //}
      //reports.forEach((report) => {
      //  Post.findOne({where: {id: report.postId}}).then(getPost => {
      //    console.log(getPost.content);
      //    //report.content = getPost.content;
      //  });
      //});
      console.log(displayContent);
      res.end(pug.renderFile('./views/bad-reports.pug', {
        reports: reports,
  ...
  case 'POST':
    ...
    req.on('data', (chunk) => {
      ...
      body = Buffer.concat(body).toString();
      const decoded = decodeURIComponent(body);
      const matchResult = decoded.match(/postId=(.*)&oneTimeToken=(.*)/);
      if (!matchResult) {
        util.handleBadRequest(req, res);
      } else {
